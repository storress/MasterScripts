...
def pl_graph(testCase):
    ...
    for paramRep in newlist:
        ...
        predsIntent = []
        for rep in paramRep:
            predsIntent.append(float(rep[-1]))
        thisGraph.append([packetLoss, statistics.mean(predsIntent)])
...
plt.figure(5)
plt.plot([row[0] for row in graph5], [row[1] for row in graph5])
plt.title('WavePacketLoss - [Car - Bike]')
plt.xlabel('Packet Loss %')
plt.ylabel('Accuracy %')
plt.grid()
plt.figure(6)
plt.plot([row[0] for row in graph6], [row[1] for row in graph6])
plt.title('WaveOnlyPacketLoss - [Car - RSU - Bike]')
plt.xlabel('Packet Loss %')
plt.ylabel('Accuracy %')
plt.grid()
plt.figure(7)
plt.plot([row[0] for row in graph7], [row[1] for row in graph7])
plt.title('WaveLtePacketLoss - [Car - RSU - (Lte) - Bike]')
plt.xlabel('Packet Loss %')
plt.ylabel('Accuracy %')
plt.grid()
plt.figure(8)
plt.plot([row[0] for row in graph8], [row[1] for row in graph8])
